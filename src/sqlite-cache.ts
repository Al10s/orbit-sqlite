...
import { supportsSQLite, log, logQuery, logMethod, stripNull } from './utils';
...
export default class SQLiteCache extends AsyncRecordCache {
  ...
  private async _parseRecordFromDb (input: SQLiteRecord, type: string, db: SQLiteDatabase): Promise<Record> {
    ...
    const processedAttributes = stripNull(attributes);
  ...
  private _setRecord (record: Record, tx: Transaction, shouldInsert: boolean, existingRelationships: Dict<RecordRelationship>): Record {
    ...
    if (shouldInsert) {
      ...
      tx.executeSql(`INSERT INTO '${type}'(${kv.map(({key}) => `'${key}'`).join(', ')}) VALUES(${placeholder.join(', ')})`, kv.map(({value}) => value));
    }
    else {
      if (kv.length > 0) {
        ...
        tx.executeSql(`UPDATE '${type}' SET ${kv.map(({key}) => `'${key}'=?`).join(', ')} WHERE id=?`, values);
      }
    }
    if (relationships !== undefined) {
      ...
      for (const [name, recordRelationship] of Object.entries(relationships)) {
        ...
        if (relationshipType === 'hasMany') {
          ...
          const recordIdentity = recordRelationship.data as RecordIdentity|null;
          if (existing === undefined || recordIdentity === null || (existing.id !== recordIdentity.id || existing.type !== recordIdentity.type)) {
            relationshipsToAdd[name] = recordRelationship;
          }
        }
      }
      for (const [ name, relationship ] of Object.entries(relationshipsToAdd)) {
        const recordIdentitites: RecordIdentity[] = Array.isArray(relationship.data) ? relationship.data : [relationship.data];
        for (const recordIdentity of recordIdentitites) {
          if (recordIdentity === null) {
            const relationshipModel = model.relationships[name].model as string;
            logQuery(
              `DELETE FROM 'relationships_${relationshipModel}_${type}' WHERE ${type}_id=?`,
              [ id ]
            );
            tx.executeSql(
              `DELETE FROM 'relationships_${relationshipModel}_${type}' WHERE ${type}_id=?`,
            ...
            logQuery(
              `DELETE FROM 'relationships_${type}_${relationshipModel}' WHERE ${type}_id=?`,
              [ id ]
            );
            tx.executeSql(
              `DELETE FROM 'relationships_${type}_${relationshipModel}' WHERE ${type}_id=?`,
            ...
            logQuery(
              `INSERT INTO 'relationships_${relationshipModel}_${type}' ('${type}_id', '${relationshipModel}_id') VALUES (?, ?)`,
              [ id, relationshipId ]
            );
            tx.executeSql(
              `INSERT INTO 'relationships_${relationshipModel}_${type}' ('${type}_id', '${relationshipModel}_id') VALUES (?, ?)`,
  ...
  async setRecordsAsync (records: Record[]): Promise<void> {
    ...
    if (records.length > 0) {
      ...
      const recordsAdded = await this.getRecordsAsync(recordsToAdd.map(({ record }) => record));
      if (this._keyMap) {
  ...
  private _removeRecord (recordIdentity: RecordIdentity, tx: Transaction) {
    ...
    tx.executeSql(`DELETE FROM '${type}' WHERE id=?`, [id]);
  ...
  async getInverseRelationshipsAsync (recordIdentity: RecordIdentity):
    ...
    if (relationshipsToAdd.length > 0) {
      await db.transaction((tx: Transaction) => {
        for (const { record, relatedRecord } of relationshipsToAdd) {
          ...
          logQuery(
            `INSERT INTO 'relationships_${relatedRecord.type}_${record.type}' ('${record.type}_id', '${relatedRecord.type}_id') VALUES(?, ?)`,
            [ record.id, relatedRecord.id ]
          )
          tx.executeSql(
            `INSERT INTO 'relationships_${relatedRecord.type}_${record.type}' ('${record.type}_id', '${relatedRecord.type}_id') VALUES(?, ?)`,
  ...
  async removeInverseRelationshipsAsync (relationships: RecordRelationshipIdentity[]): Promise<void> {
    ...
    await db.transaction((tx: Transaction) => {
      for (const { record, relatedRecord } of relationships) {
        ...
        logQuery(
          `DELETE FROM 'relationships_${relatedRecord.type}_${record.type}' WHERE ${record.type}_id=? AND ${relatedRecord.type}_id=?`,
          [ record.id, relatedRecord.id ]
        )
        tx.executeSql(
          `DELETE FROM 'relationships_${relatedRecord.type}_${record.type}' WHERE ${record.type}_id=? AND ${relatedRecord.type}_id=?`,
